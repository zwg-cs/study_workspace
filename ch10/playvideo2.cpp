...
int main(int argc, char **argv)
{
    ...
    {
        av_log(nullptr, AV_LOG_ERROR, "can't initialize SDL\n");
        return -1;
    }
    // 创建SDL窗口
    SDL_Window *window = SDL_CreateWindow("Video player",
                                          SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, video_decode_ctx->width, video_decode_ctx->height,
                                          SDL_WINDOW_OPENGL | SDL_WINDOW_RESIZABLE);
    if (!window)
    {
        av_log(nullptr, AV_LOG_ERROR, "can not create window\n");
        return -1;
    }
    ...
    if (!renderer)
    {
        av_log(nullptr, AV_LOG_ERROR, "can not create renderer\n");
        return -1;
    }
    // 创建SDL纹理
