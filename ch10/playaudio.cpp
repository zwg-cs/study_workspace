...
#define MAX_AUDIO_FRAME_SIZE 8096   // 一帧音频最大长度（样本数）
...
{
    const char *src_name = "E:\\MyProjects\\av\\plum.mp3";
    if (argc > 1)
    {
        src_name = argv[1];
    }
    ...
    int out_sample_rate = 44100;                              // 输出采样频率
    int out_nb_samples = audio_decode_ctx->frame_size;        // 输出的采样数量
    int out_channels = out_ch_layout.nb_channels;             // 输出的通道数
    if (out_nb_samples == 0)
    {
    ...
    SwrContext *swr_ctx = nullptr;                           // 音频采样的实例
    ...
    while (av_read_frame(in_fmt_ctx, packet) >= 0) // 轮询数据包
    {
        if (packet->stream_index == audio_index)
        {
            ...
            if (ret == 0)
            {
                ...
                swr_convert(swr_ctx,                                         // 音频采样器实例
                            &out_buff, MAX_AUDIO_FRAME_SIZE,                 // 输出数据内容和数据打下
                            (const uint8_t **)frame->data, frame->nb_samples // 输入数据内容和数大小
                );
                // 把音频数据同步到缓冲区位置
                audio_pos = (unsigned char *)out_buff;
                // 缓冲区大小
