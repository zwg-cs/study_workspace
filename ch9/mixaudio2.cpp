...
int main(int argc, char **argv)
{
    ...
    int64_t last_pts = 0;      // 最近的时间戳
    int64_t increment_pts = 0; // 时间戳增量
    int i = 0;
    int ret = -1;
    ...
    // 如果第一路音频已经遍历完，第二个音频还有数据，处理第二个音频
    while (av_read_frame(in_fmt_ctx[1], packet[1]) >= 0)
    {
        if (packet[1]->stream_index == audio_index[1])
        {
            ret = avcodec_send_packet(audio_decode_ctx[1], packet[1]);
            if (ret == 0)
            {
                ret = avcodec_receive_frame(audio_decode_ctx[1], frame[0]);
                if (ret == AVERROR(EAGAIN) || ret == AVERROR_EOF)
                {
                    break;
                }
                ...
                if (ret == 0)
                {
                    while (1)
                    {
                        ...
                        filter_frame->pts = last_pts; // 使用第一个文件的时间戳
                        output_audio(filter_frame);
                    }
                }
                else
                {
