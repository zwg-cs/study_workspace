...
int recode_video(AVPacket **packet, AVFrame **frame, AVFrame *filt_frame)
{
    ...
    while (1)
    {
        ...
        if (ret == 0) // 如果第二个文件没有到末尾，把第二个文件的数据帧添加到相应的滤镜区域
        {
        ...
        while (1)
        {
            ...
            if (second_end != 1) // 第二个文件没有结束
            {
                output_video(filt_frame);
            }
            else // 第二个文件已经结束
            {
                frame[0]->pts = filt_frame->pts;
                output_video(frame[0]);
            }
        }
    }
    return ret;
...
int main(int argc, char **argv)
{
    ...
    {
        if (packet[1]->stream_index == video_index[1])
        { // 视频包需要重新编码
