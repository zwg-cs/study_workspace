...
AVFilterContext *abuffersrc_ctx = nullptr;  // 输入滤镜的实例
AVFilterContext *abuffersink_ctx = nullptr; // 输出滤镜的实例
AVFilterGraph *afilter_graph = nullptr;     // 滤镜图
...
int init_filter(const char *filters_desc)
{
    ...
    inputs->next = nullptr;
    // 把采用过滤字符串描述的图形添加到滤镜图
    ret = avfilter_graph_parse_ptr(afilter_graph, filters_desc, &inputs, &outputs, nullptr);
    if (ret < 0)
    {
...
int main(int argc, char **argv)
{
    ...
    int is_atempo = (strstr(filters_desc, "atempo=") != nullptr);
    ...
    packet->size = 0;
    recode_audio(packet, frame, filt_frame); // 对音频帧重新编码
    output_audio(nullptr);                   // 传入一个空帧，冲走编码缓存
    av_write_trailer(out_fmt_ctx);           // 写文件尾
    av_log(nullptr, AV_LOG_INFO, "Success process audio file.\n");
