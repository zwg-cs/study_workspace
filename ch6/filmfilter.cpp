...
int main(int argc, char **argv)
{
    ...
    {                                             // 构造老电影上下两侧的胶卷边框滤镜
        ...
        int add_height = 140;                     // 增加的高度
        int side = 30;                            // 方块的边长
        int gap = 20;                             // 方块的间隔
        int box_count = width / (side + gap);     // 方块的数量
        char film_desc[(box_count + 1) * 60 * 2]; // 老电影的过滤字符串
        int pos = 0;                              // 记录当前的写入位置
        pos += snprintf(film_desc + pos, sizeof(film_desc) - pos, "pad=w=iw:h=ih+%d:x=0:y=%d:color=black", add_height, add_height / 2);
        int i = 0;
        while (i <= box_count) // 往上下两侧新增的黑边添加白色小方块
        {
            int x_pos = gap + i * (side + gap);
            int x_side = x_pos + side > width ? width - x_pos : side;
            pos += snprintf(film_desc + pos, sizeof(film_desc) - pos, ",drawbox=x=%d:y=%d:w=%d:h=%d:color=white:t=fill", x_pos, gap, side, side);
            pos += snprintf(film_desc + pos, sizeof(film_desc) - pos, ",drawbox=x=%d:y=ih-%d:w=%d:h=%d:color=white:t=fill", x_pos, side + gap, side, side);
            ++i;
        }
