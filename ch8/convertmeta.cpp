...
int is_iso(unsigned char *data, int len)
{
    ...
    else if ((data[0] == 0xc2 || data[0] == 0xc3) && (data[2] == 0xc2 || data[2] == 0xc3))
    {
        return 0;
    }
    else
...
void get_zh(char *key, char *src_str, char *dst_str, size_t dst_len)
{
    ...
    { // 采用GBK编码，要改成Linux默认的UTF-8
        printf("Linux GBK\n");
        gbk_to_utf8(tmp_str, tmp_len, dst_str, dst_len);
    }
#endif
    printf("metadata %s=%s, length=%ld, is_iso=%s, is_gbk=%s, is_utf8=%s\n",
