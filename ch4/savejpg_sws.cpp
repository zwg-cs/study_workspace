...
int save_jpg_file(AVFrame *frame, int save_index)
{
    ...
    char jpg_name[20] = {0};
    sprintf(jpg_name, "output_yuvj_%03d.jpg", save_index);
    av_log(nullptr, AV_LOG_INFO, "target image file is %s.\n", jpg_name);

    enum AVPixelFormat target_format = AV_PIX_FMT_YUVJ420P; // jpeg的像素格式为YUVJ420P
    ...
    yuvj_frame->format = target_format;
